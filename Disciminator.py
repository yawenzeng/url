...
class Disciminator(nn.Module):
    def __init__(self):
        ...
        self.sr1 = torch.nn.Conv2d(1024, 4800, kernel_size=1, stride=1)
        self.sr2 = torch.nn.Conv2d(1024, 2048, kernel_size=1, stride=1)
    ...
    def forward(self, visual_feature_train, sentence_embed_train):
        ...
        sr = (self.sr1(transformed_sentence_train_norm)-sentence_embed_train).pow(2).squeeze().mean()+self.sr2(transformed_clip_train_norm)-visual_feature_train).pow(2).squeeze().mean()
        consistency=(transformed_clip_train_norm - transformed_sentence_train_norm).pow(2).squeeze().mean()+sr
